...
public abstract class UniqueIdAuthProvider<R, UL extends UniqueIdAuthUser, US extends UniqueIdAuthUser, L extends UniqueIdAuthProvider.UniqueId, S extends UniqueIdAuthProvider.UniqueId>
	...
	protected static final String PROVIDER_KEY = "uniqueid";

	protected static final String SETTING_KEY_MAIL = "mail";

	protected Mailer mailer;

	public UniqueIdAuthProvider(final Application app) {
		super(app);
	}
	...
	public AuthUser getSessionAuthUser(final String id, final long expires) {
		return new SessionUniqueIdAuthUser(getKey(),
				NumberUtils.createLong(id), expires);
	}
	 ...
	 * implementation, however the default should be fine for most cases
	 * 
	 * @param user
	 * @return
	 */
	protected String getEmailName(final US user) {
		...
		if (user instanceof FirstLastNameIdentity) {
			firstName = ((FirstLastNameIdentity) user).getFirstName();
		}
		String lastName = null;
		if (user instanceof FirstLastNameIdentity) {
			lastName = ((FirstLastNameIdentity) user).getFirstName();
		}

		return getEmailName(user.getEmail(), firstName + " " + lastName);
	}
	 ...
	 * @return
	 */
	protected abstract UL transformAuthUser(final US authUser,
			final Context context);

	protected abstract Call userExists(final UniqueIdAuthUser authUser);

	protected abstract Call userUnverified(final UniqueIdAuthUser authUser);
	...
	protected enum LoginResult {
		NOT_FOUND, USER_LOGGED_IN, USER_UNVERIFIED, WRONG_PASSWORD
	}

	protected enum SignupResult {
		USER_CREATED, USER_CREATED_UNVERIFIED, USER_EXISTS, USER_EXISTS_UNVERIFIED
	}
